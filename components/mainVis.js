...
$(document).ready(function () {
    ...
    function stage_2(){
        ...
        if (parseFloat(wordSvg.attr('opacity')) === 0){
            ...
            cityBridgeSvg.selectAll()
                ...
                .attr('y', offsetY + 50)
                .attr('transform', (d, i) => `rotate(-45 ${i * opt_cityBridge.dur + offsetX + 12},${offsetY + 50})`)
                .text(d=>d.name)

            cityBridgeSvg.selectAll('#province')
                .data(["Province"])
                .join('text')
                .attr('id', 'province')
                .attr('fill', 'white')
            ...
            if (lastCities.indexOf(provincesMap[d.name]) != -1) {
                let city = d3.select(this)
                    ...
                    .on('mouseover', (e,d)=>{
                        ...
                        let data = getFilterData(filter, dragDate)
                        for (let k in data) {
                            if (provincesMap[k] === provincesMap[d.name]){
                                data = data[k];
                                break
                            }
                        }
    ...
    progressLoaded('Mainly Visual Component')
})
